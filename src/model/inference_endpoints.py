...
class LLM:
    """Class implementation of Hugging Face models using AutoModel and AutoTokenizer for inference."""
    def __init__(
        self,
        model: str,
    ...
    def answer_message_queue(
        ...
        max_new_tokens = map_reduce_agg_max_new_tokens or self.gen_params.get('MAX_NEW_TOKENS', 512)

        inputs = '\n'.join([msg['content'] for msg in message_queue])
        inputs = self.tokenizer(inputs, return_tensors="pt", truncation=True).to(self.model.device)
        try:
    ...
    def __call__(
        ...
        supported_response_types = ['single', 'map', 'map_reduce', 'refine', 'reduce']
        assert response_type in supported_response_types, f"Response {response_type} is not supported. Choose from {supported_response_types}"
        
        max_new_tokens = self.gen_params.get('MAX_NEW_TOKENS', 512)
        message_queue = [{'role': 'system', 'content': SYSTEM_PROMPT.format(max_new_tokens=max_new_tokens)}]

        if response_type in ['single', 'refine']:
            if isinstance(prompts, list):
                assert len(prompts) == 1, "Single/Refine methods should only have one prompt"
                prompt = prompts[0]
            else:
            ...
            message_queue = self.LLM_turn(message_queue, new_prompt=prompt)
